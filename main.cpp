...
void InitFloor(MESH *object) {
    VERTEX g_vertex_buffer_data[] = {
        //{{x,y,z},   {r,g,b,a}}
        /* TOP */
        {{ 20.0,  -20.0,  20.0},  {1, 0, 1, 1}, {0,1,0}}, //FRONT RIGHT
        {{ 20.0,  -20.0, -20.0},  {1, 0, 1, 1}, {0,1,0}}, //REAR RIGHT
        {{-20.0,  -20.0, -20.0},  {1, 0, 1, 1}, {0,1,0}}, //REAR LEFT
        {{-20.0,  -20.0,  20.0},  {1, 0, 1, 1}, {0,1,0}}, //FRONT LEFT
...
int main(int argc, char * argv[]){
    ...
    while (running)
    {
        ...
        if( currentKeyStates[ SDL_SCANCODE_W ] ) {
            vec3 resultant;
            vec3 delta;
            vec3_scale(delta,plane_front,(float)delta_frame_time * 1.0f / 1000);
        ...
        if( currentKeyStates[ SDL_SCANCODE_A ] ) {
            vec4 resultant;
            vec3 delta;
            vec3_scale(delta,plane_right,(float)delta_frame_time * 1.0f / 1000);
            vec3_sub(resultant,plane_pos,delta);
            vec3_dup(plane_pos,resultant);
        }
        ...
        if(  currentKeyStates[ SDL_SCANCODE_C ] ) {
            vec4 resultant;
            vec3 delta;
            vec3_scale(delta,plane_up,(float)delta_frame_time * 1.0f / 1000);
            vec3_sub(resultant,plane_pos,delta);
            vec3_dup(plane_pos,resultant);
        }
